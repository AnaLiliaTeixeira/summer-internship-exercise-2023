...
class SnailShellPattern implements ISnailShellPattern {
  ...
  public Future<int[]> getSnailShell(int[][] matrix) {
    ...
    while (orderIndex < order.length) { // Clockwise direction : right -> down -> left -> up

      // Traverse the elements by direction right
      for (int j = 0; j < matrix[0].length; j++) {
        traverseRowElements(matrix, already_persued, order, j);
      }

      // Traverse the elements by direction down
      for (int i = 0; i < matrix.length; i++) {
        traverseColumnElements(matrix, already_persued, order, i);
      }

      // Traverse the elements by direction left
      for (int j = matrix[0].length - 1; j >= 0; j--) {
        traverseRowElements(matrix, already_persued, order, j);
      }

      // Traverse the elements by direction up
   ...
   * 
   * @param matrix          matrix of numbers to go through
   * @param already_persued boolean matrix that indicates whether the element was
   *                        persued or not
